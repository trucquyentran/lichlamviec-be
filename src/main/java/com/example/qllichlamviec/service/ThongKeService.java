...
import com.example.qllichlamviec.util.pojo.CheckTypeId;
...
public class ThongKeService {
    ...
    @Autowired
    private CheckTypeId checkIdType;
    ...
    public List<ThongKeSLLoaiLichCuaMotDonViHoacNhanVienDTO> countCateEventOfUser(ObjectId userId) {
        ...
        AggregationResults<ThongKeSLLoaiLichCuaMotDonViHoacNhanVienDTO> results = mongoTemplate.aggregate(aggregation, "LichLamViec", ThongKeSLLoaiLichCuaMotDonViHoacNhanVienDTO.class);
        return results.getMappedResults();
    }
...
//    }
    ...
    public List<ThongKeSLLoaiLichCuaMotDonViHoacNhanVienDTO> countCateEventOfUser(ObjectId id, Date startDate, Date endDate) {
        Date end = endDate;
        Calendar c = Calendar.getInstance();
        c.setTime(end);
        c.add(Calendar.DATE, 1);
        end = c.getTime();
        String idType = checkIdType.checkIdType(id);
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where(idType).is(id)
                        .and("thoiGianBD").gte(startDate)
                        .and("thoiGianKT").lte(end)),
                Aggregation.lookup(idType.equals("taiKhoan") ? "TaiKhoan" : "DonVi", idType, "_id", "Info"),
